...
_HTABLE_T _leaker = { NULL, 0, 0, 0, 0, 0, 0, 0, 0, 0 };
...
void _Leaker_Dump(void)
{
    ...
    else
    {
        ...
        unsigned int i;
		std::cout << _leaker.count << " allocations (" << _leaker.bytes - _leaker.count * GUARD_SIZE
			<< " bytes) in table of " << _leaker.rows << " rows.\n";
    ...
    if(_leaker.mismatches)
		std::cout << "Mismatches: " << _leaker.mismatches << " allocation/deallocations don't match!\n";
	if (_leaker.overflows)
		std::cout << "Overflows: " << _leaker.overflows << " allocations overflowed (wrote off end)!\n";
	if (_leaker.bad_frees)
		std::cout << "Bad deallocs: " << _leaker.bad_frees << " attempts made to deallocate unallocated pointers!\n";
                     ...
                     unsigned long line)
{
    ...
    if(!(ptr = malloc(size)))
    {
		std::cout << file << ":" << func << "():" << line << " aborting: malloc() failed!" << std::endl;
        exit(2);
    }
                     ...
                     const char *func, unsigned long line)
{
    ...
    {
		std::cout << file << ":" << func << "():" << line << " aborting: calloc() failed!" << std::endl;
        exit(2);
    }
                      ...
                      const char *func, unsigned long line)
{
    ...
    {
		std::cout << file << ":" << func << "():" << line << " aborting: realloc() failed!" << std::endl;
        exit(2);
    }
...
void* operator new (size_t size)
{
    ...
    if(!(ptr = malloc(size)))
    {
		std::cout << _leaker_file << ":" << _leaker_func << "():" << _leaker_line << " aborting: new failed!" << std::endl;
        exit(2);
    }
...
static void _Leaker_Init(void)
{
    ...
    if(!_leaker.table)
    {
		std::cout << "aborting: calloc() for table failed!" << std::endl;
        exit(2);
    }
                        ...
                        const char *file, const char *func, size_t line)
{
    ...
    {
		std::cout << file << ":" << func << "():" << line << "fatal error: address " << addr << " already in use!" << std::endl;
        exit(2);
    }
    
    if(!(temp = (_LEAK_T *)malloc(sizeof(_LEAK_T))))
    {
		std::cout << "aborting: malloc() for entry failed!" << std::endl;
        exit(2);
    }
    ...
    *mover = temp;
	...
	{
		temp->unknown = true;
		_leaker.unknown_count++;
		_leaker.unknown_bytes += size;
	}
}
                            ...
                            const char *func, size_t line)
{
    ...
    {
		std::cout << "\nLEAKER: " << file << " : " <<
			func << "() : " << line << " " << dealloc << " error : pointer was not allocated!\n" << std::endl;
        _leaker.bad_frees++;
        return 0;
    }
    ...
    *mover = temp->next;
	...
	{
		_leaker.unknown_count--;
		_leaker.unknown_bytes -= temp->size;
	}
    ...
    {
		std::cout << "\nLEAKER: " << file << ":" << func << "():" << line <<
			" checking error: wrote off end of memory allocated at " << temp->file << ":" <<
			temp->func << "():" << temp->line << ".\n" << std::endl;
        _leaker.overflows++;
    }
    ...
    {
		std::cout << "\nLEAKER: " << file << ":" << func << "():" << line <<
			" mismatch error: memory allocated at " << temp->file << ":" <<
			temp->func << "():" << temp->line << " with " << temp->alloc << 
			", deallocated with " << dealloc << ".\n" << std::endl;
        _leaker.mismatches++;
    }
...
static void _Leaker_Report(void)
{
	...
	if ((_leaker.count - _leaker.unknown_count) == 0)
	{
		if(_leaker.table) free(_leaker.table);
    ...
    if(_leaker.count) /* print out list of leaks, clean up */
    {
        ...
        /*std::cout << "Leaks found: " << _leaker.count - _leaker.unknown_count << " allocations (" <<
			_leaker.bytes - _leaker.unknown_bytes - _leaker.count * GUARD_SIZE << " bytes).\n";*/
        
		unsigned actual_leaks = 0;
		...
		if (actual_leaks > 0) {
			std::cout << "\nLEAKER: MEMORY LEAKS: " << actual_leaks << " leaking " << actual_bytes << " bytes! " << std::endl;
		}
        //free(table);
    }
    ...
    /* report other errors */
    if(_leaker.mismatches)
		std::cout << "\nLEAKER: MISMATCHES: " << _leaker.mismatches << " allocation/deallocations don't match.\n";
    
    if(_leaker.overflows)
        std::cout << "\nLEAKER: OVERFLOWS: " << _leaker.overflows << " allocations overflowed (wrote off end).\n";

    if(_leaker.bad_frees)
		std::cout << "\nLEAKER: BAD DEALLOCS: " << _leaker.bad_frees << " attempts made to deallocate unallocated pointers.\n";
...
static void _Leaker_Print_Entry(_LEAK_T *entry)
{
	if (strcmp(entry->file, "unknown") == 0)
	{
		return;
	}

	const char* baseFile = strrchr(entry->file, '/');
	if (!baseFile)
	{
		baseFile = strrchr(entry->file, '\\');
	}
	baseFile++;

	//std::cout << baseFile << ":" << entry->func << "():" << entry->line << " memory leak: memory was not deallocated." << std::endl;;
    if(!_Leaker_Check_Guard(entry->addr, entry->size))
    {
		std::cout << entry->file << ":" << entry->func << "():" << entry->line << 
			" checking error: wrote off end of allocation." << std::endl;
        _leaker.overflows++;
    }
...
{
	std::cout << entry->file << ":" << entry->func << "():" << entry->line <<
		" address: " << entry->addr << " bytes: " << entry->size - GUARD_SIZE;
    if(!_Leaker_Check_Guard(entry->addr, entry->size))
    {
...
static _LEAK_T **_Leaker_Build_List(void)
{
    ...
    {
		std::cout << __FILE__ << ":" << __func__ << "():" << __LINE__ << " aborting: malloc() failed!" << std::endl;
        exit(2);
    }
